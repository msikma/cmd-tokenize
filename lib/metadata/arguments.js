...
const { reprObject, isString } = require('../util')
...
const findTerminator = (arg, options, format, isExec = false, isAfterTerminator = false) => {
  if (!options.useOptionsTerminator) return null
  if (isExec || isAfterTerminator) return false
...
const unpackCombinedOptions = (argObject, needsUnpacking) => {
  ...
  // An important detail is that only the last item takes any suffix that might exist,
  // so an argument like -ab="c" is parsed as though it's -a -b="c".
  return [...argObject.content].map((char, idx, arr) => {
    return {
      ...argObject,
      suffix: idx === arr.length - 1 ? argObject.suffix : null,
      content: char,
      isUnpacked: true
    }
  })
 ...
 * 
 * The rest of the code does not call this function, so we check to see if the input is a string
 * and we merge the defaults into the user's options. It's exposed to the user as util.parseArgument().
 */
const parseArgument = (arg, userOptions, userFormat, isExec = null, isAfterTerminator = null) => {
  if (!isString(arg)) {
    throw new ParseError(`parseArgument: Argument must be a string: ${reprObject(arg)}`)
  }
  const { options, format } = mergeDefaults({ firstIsExec: false, useOptionsTerminator: false, ...userOptions }, userFormat)
...
const argumentMetadata = (arg, options, format, isExec, isAfterTerminator) => {
  ...
  const prefix = findPrefix(arg, format.optionDelimiters, isStatic)
  ...
  const suffix = findSuffix(prefix, format.valueDelimiters, isStatic)
