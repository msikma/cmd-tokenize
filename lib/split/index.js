...
const { isWhitespace, isEscapeChar, isQuoteChar, isString, isArray, reprObject } = require('../util')

/**
 * Returns an array of distinct argument segments from the input.
 * 
 * If the input is a string, the string is split by whitespace and unescaped.
 * If an array is passed, the array is returned verbatim; in this case it's assumed
 * that all proper unescaping has already been done.
 */
const getArgumentSegments = (input, options) => {
  if (isArray(input)) {
    return input
  }
  return splitArgumentSegments(input, options)
}

/**
 * Takes a command string and returns an array of distinct argument segments.
 * 
 * A command is anything the user may type into a terminal, such as `ls -lah`
 * or `find -type f -name '*.js'`. The argument segments are the individual words
 ...
 */
const splitArgumentSegments = (input, options) => {
  let char = null
  let index = null
  ...
  /**
   * Yields all argument segments from the input.
   * 
  ...
  function* yieldArguments() {
    ...
    while (char) {
      if (isQuoteChar(char)) {
        yield joinSegment(yieldQuote(), char)
      }
      else {
        yield joinSegment(yieldWord())
      }
  ...
  const parseInput = () => {
    if (!isString(input)) {
      throw parseError(`splitArgumentSegments: Input must be a string: ${reprObject(input)}`)
    }
