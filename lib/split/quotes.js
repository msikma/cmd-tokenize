...
const { findPrefix, findTerminator } = require('../arguments/meta')
 ...
 */
const mergeConsecutiveArguments = (flatItems) => {
  let items = []
  let consecutive = []
...
/**
 * Modifies the split items to handle arguments with separators. TODO: fix description, suffix = separator
 * 
...
const handleSeparators = (flatItems, options, format) => {
  ...
  for (const item of flatItems) {
    ...
    const prefix = findPrefix(item, format.valuePrefix, isTerminator || afterTerminator)
    ...
    if (!prefix.metadata.isOption || afterTerminator) {
      items.push(item)
      continue
    }
    ...
    for (let n = 0; n < format.valueSuffix.length; ++n) {
      ...
      items.push(`${split[0]}${sep}`)
      items.push(split.slice(1).join(sep))
      break
    }
  }
