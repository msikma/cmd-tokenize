...
const { isString, reprObject } = require('./util')
const { ParseError } = require('./error')
const { splitArgumentBlocks } = require('./split')
...
const commandParse = (callerName, doFullParsing, overrideOptions = {}) => (cmd, userOptions, userFormat) => {
  if (!isString(cmd)) {
    throw new ParseError(`${callerName}: Command must be a string: ${reprObject(cmd)}`)
  }

  const { options, format } = mergeDefaults({ ...userOptions, ...overrideOptions }, userFormat)
  const args = splitArgumentBlocks(cmd, options)

  // If only splitting, return the argument blocks only.
  if (!doFullParsing) {
    return args
  }
...
/**
 * Splits a command string up into an array of argument blocks.
 * 
